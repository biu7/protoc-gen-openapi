...
// findCommentSummary find the first sentence of a comment.
func (g *OpenAPIv3Generator) findCommentSummary(c string) string {
	splitComment := strings.Split(c, " ")
			...
			for _, rule := range rules {
				...
				if methodName != "" {
					...
					op, path2 := g.buildOperationV3(
						d, operationID, service.GoName, summary, comment, defaultHost, path, body, inputMessage, outputMessage)
